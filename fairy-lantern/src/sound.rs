...
//!
//! Simple design that matches hardware well enough for commercial ROMs:
//! - Clock both FIFOs from the selected sample timer(s) (SOUNDCNT_H bits 10/14).
//! - Emit **one mixed sample per sample-timer tick** at the game's rate.
//! - Hand the raw stream to aplay/pw-cat; the host device resamples.
//! - Underrun → silence for that channel (no held/sticky sample).
...
/// Only used before the game programs a sample timer (or if PSG alone is live).
const DEFAULT_RATE: u32 = 32768;
/// Live host ring (~0.5 s @ 48 kHz).
const RING_CAP: usize = 48_000 / 2;
/// Rolling capture for WAV dump (~4 s @ 32 kHz).
const CAPTURE_CAP: usize = 32_768 * 4;
...
pub struct Sound {
    ...
    fifo_b: VecDeque<i8>,
    psg: Psg,
    /// Accumulator for the primary sample clock (shared mix rate).
    cycle_accum: u32,
    /// CPU cycles per output sample (from the active FIFO timer).
    cycles_per_sample: u32,
    /// Rate published to the host player / WAV dump.
    pub stream_rate: u32,
    last_mix: i16,
    pub samples_out: u64,
...
impl Sound {
    pub fn new() -> Self {
        Self {
            ...
            psg: Psg::default(),
            cycle_accum: 0,
            cycles_per_sample: GBA_CLOCK / DEFAULT_RATE,
