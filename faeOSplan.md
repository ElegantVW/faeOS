...
- **2026-08-12 (seal v0.3 + hearth)** — Renamed pixie-lock → seal across entire codebase. Built **hearth**: user manager daemon (Rust) listening on Unix socket `/tmp/hearth.sock`. Discovers users from `/etc/passwd`, handles multi-user auth via PTY-based `su`, session switching via `loginctl activate`. Protocol: simple line-delimited JSON (LIST/AUTH/LOGIN/SESSIONS/PING). Updated seal for multi-user UI: queries hearth on startup, shows user list with ↑/↓ selection, authenticates against selected user, auto-switches session on success. Falls back to local `sudo` auth when hearth unavailable. Systemd units for both seald.service and hearth.service. Configs renamed: lock.json→seal.json, pixie-lock.pam→seal.pam. Binary names: seal, seald, seal-tui, hearth, hearth-guest.
