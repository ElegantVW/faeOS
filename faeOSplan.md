...
- **2026-08-12 (pixie-lock v0.2 — native X11)** — Rewrote pixie-lock as a native X11 screen locker (drops i3lock dependency). Uses `x11rb` for direct X11 protocol: creates override-redirect fullscreen windows, grabs keyboard/pointer, renders pixel buffer with MIT-Shm-style chunked PutImage. Animated UI with live clock, floating sparkle particles, pulsing fae sigil, styled password input box with error glow, Caps Lock warning, battery status bar. PAM auth via `su` fallback with optional `/etc/pam.d/pixie-lock` service. US keycode-to-character mapping for keyboard input. Clean ungrab+destroy on exit.
