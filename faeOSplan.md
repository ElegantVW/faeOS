...
- **2026-08-12 (pixie-lock)** — Built faeOS lock screen system in Rust. Three components: `pixie-lock` (elaborate themed lock image generator + i3lock wrapper), `pixie-lockd` daemon (idle watcher with battery-aware timeouts, xprintidle polling, config hot-reload), `lock-tui` (Python configurator with timeout/guest/toggle settings). Added `fae-guest` helper (temporary restricted user for lock screen guest mode — auto-created/wiped, no sudo, locked-out of home dirs). Image features: dark pink grain background, CRT scanlines, fae sigil, centered digital clock, date, custom message, battery status bar, guest hint. Systemd user unit `pixie-lockd.service` auto-starts at boot. Dependencies: xprintidle, i3lock (added to pkglist). Integrated into install.sh, fae-bootstrap.sh, and i3 config ($mod+Ctrl+l to lock).
